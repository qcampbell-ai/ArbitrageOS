...
|---|---|---|
| **GitHub** (this repo) | Official source of truth | PRD, Company Bible, Architecture, Roadmap, ADRs, AI agent prompts, code, SOPs |
| **Google Sheets** | Operational system of record | Live business data: Dashboard, KPIs, Products, Inventory, Purchases, Sales, Suppliers, Expenses, Daily Log, Scoreboard, Decision Framework |
...
├── sops/                       ← (existing — see note below)
├── finance/                     ← (existing — see note below)
└── sourcing/                     ← (existing — see note below)
```

> **Note on the folders above `prompts/`:** these were created in Sprint 1
> and are intentionally left as-is here. A later ADR should decide whether
> to rename/consolidate them — that decision hasn't been made yet, so this
> README doesn't presume an answer.
